...
export class ReportsService {
  ...
  private async generatePdfReport(bookings: BookingHistory[]): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      if (bookings.length === 0) {
        doc
          .fontSize(14)
          .text('No bookings available for the selected filters.');
      } else {
  ...
  private async generateXmlReport(bookings: BookingHistory[]): Promise<Buffer> {
